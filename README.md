...
```mermaid
flowchart LR
	DJ[DJ] -->|Enters show details to spreadsheet | sheet[Google Sheet]
	radioPC[Radio PC] -->|Uploads radio recording| drive[Google Drive folder]
	sheet -->|Show details matched by time from spreadsheet | uploader[Recording uploader]
	drive -->|New audio file detected| uploader
	uploader -->|Uploads show and metadata| soundcloud[SoundCloud]
	soundcloud -->|Show URL returned once complete| uploader
	uploader -->|Sends show upload notification with URL via email| DJ
```
